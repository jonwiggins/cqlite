...
Every component — the hand-written SQL tokenizer, recursive descent parser, B-tree storage engine, page cache, rollback journal, and query executor — was implemented from first principles without copying from the SQLite source or using any existing SQL parsing libraries or database engines. Only 2 external crates are used: `thiserror` for error types and `tempfile` for tests.
